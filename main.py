...
AI_SYSTEM_PROMPT = (
    ...
    "ГЛАВНОЕ: Твоя задача — быть актуальным, умным и полезным Destiny 2 ассистентом, Лучше дать короткий, точный и актуальный ответ, чем длинный и устаревший. Если в твоём ответе будет фигурировать слова: VPN/ВПН, то ЗАМЕНЯЙ его на слово 'КВН' (такое правило в чате)"
)
...
async def join_cup_command(message: types.Message):
    ...
    if current_count < needed:
        ...
        await message.answer(
            f"🚫 НАБОР ЗАКРЫТ! Стартовая сетка сформирована.\n\n"
            f"{pairs_text}\n\n"
